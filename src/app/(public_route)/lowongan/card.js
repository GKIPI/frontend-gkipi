...
export default function Card({ array, type, onClicked }) {
  ...
  return (
    <div className="grid gap-2 sm:gap-5 grid-cols-2 lg:grid-cols-3 mx-0 p-[5%] xs:p-5 sm:p-8 md:p-12 w-full overflow-x-hidden overflow-y-hidden">
      {array.map((card, i) => (
        <div onClick={() => onClicked(card)} key={i} className="max-h-[370px] w-full shadow-md p-4 flex  justify-between flex-col">
          <div className="bg-slate-300 max-h-[250px] w-full overflow-y-hidden flex items-center justify-center">
            {isImage(card?.image) ?
              <img src={card.image} alt="Preview" />
              :
              <div className="md:min-h-[200px] flex items-center justify-center">
